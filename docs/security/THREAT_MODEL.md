...
| TH-2 | Target ambiguity routes work to the wrong agent, workspace, or project. | Private-data exposure, wrong action, lost accountability. | Target field required; deterministic target resolver contract and local tests now define ambiguity rejection, route preview, audit evidence, and wrong-destination fail-closed behavior. | Live adapters must bind trusted receipts to the resolved route before Gate 2/Gate 3 use. | Started |
...
| TH-7 | Chat-visible answer is completed privately only. | User-visible failure, audit gap, operational confusion. | Delivery expectation required; dry-run receipts and trusted live-adapter receipt model define bound delivery evidence. | Stuck/private-only recovery scanner remains Gate 2/Gate 3 work. | Started |
...
| TH-11 | Attacker floods bus with messages or claims work to starve legitimate tasks. | Denial of service, delayed operations. | Shared-use controls contract and local verifier fixtures define required rate limits, quotas, claim authorization, and stale-work monitoring. | Queue isolation and live enforcement remain Gate 2/Gate 3 work. | Started |
...
| TH-2 | Partly mitigated for explicit ambiguous targets, target-claim mismatch, deterministic local route resolution, route preview binding, adapter-surface mismatch, and wrong-destination dry-run rejection. Live trusted receipt enforcement remains Gate 2/Gate 3 work. | `docs/architecture/TARGET_RESOLVER_CONTRACT.md`, `src/agent_bus/target_resolver.py`, `tests/test_target_resolver.py`, `tests/test_dry_run.py`, `tests/fixtures/adapter_dry_runs/`, `tests/fixtures/agent_bus/envelopes/invalid/wrong-destination-delivery.json`, `tests/fixtures/agent_bus/transitions/invalid/target-claim-mismatch.json` |
...
| TH-7 | Partly mitigated by delivery expectation, wrong-destination checks, dry-run receipt validation, and trusted live-adapter receipt fixture validation. Live recovery scanning remains Gate 2/Gate 3 work. | `tests/fixtures/agent_bus/envelopes/valid/valid-notify-minimal.json`, `tests/fixtures/agent_bus/envelopes/invalid/wrong-destination-delivery.json`, `docs/architecture/LIVE_ADAPTER_DRY_RUN_CONTRACT.md`, `docs/architecture/TRUSTED_LIVE_ADAPTER_RECEIPT_MODEL.md`, `tests/fixtures/live_receipts/` |
...
| TH-13 | Partly mitigated by open-source governance files, release security gate, package provenance controls, and release evidence template. Public visibility and package release remain blocked until branch protections, CI/release automation, candidate-specific SBOM/provenance artifacts, and approval evidence are complete. | `LICENSE`, `CONTRIBUTING.md`, `SECURITY.md`, `docs/release/OPEN_CORE_RELEASE_CHECKLIST.md`, `docs/release/RELEASE_SECURITY_GATE.md`, `docs/release/PACKAGE_PROVENANCE_CONTROLS.md`, `docs/release/RELEASE_EVIDENCE_TEMPLATE.md`, `docs/operations/PROMOTION_GATE.md` |
...
- OF-11: Initial risk, control, and evidence registers plus Class 4 production-readiness gates are defined; enterprise policy index and customer-facing posture evidence remain missing.
